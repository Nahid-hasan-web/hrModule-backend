...
const get_leaves = async (req,res)=>{
    try{
        ...
        const leaves = await leaveModel.find().sort({createdAt:-1}).skip(skip).limit(limit);
       // ------------ sending response
        res.status(200).json({ success_status: "success", data: leaves , message:"Leaves fetched successfully" , page , limit , skip });

        }catch (err) {
        console.error("create  get_leaves controller ", err);
        res.status(500).json({success_status: "failed",message: "Internal server error from  leaveController/get_leaves",});
  }
}
